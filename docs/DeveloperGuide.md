...
* [`src/cljs/sablier_metamask_demo/`](src/cljs/sablier_metamask_demo/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/sablier_metamask_demo/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/sablier_metamask_demo/`](test/cljs/sablier_metamask_demo/): test files (ClojureScript,
...
7. Clone this repo and open a terminal in the `sablier-metamask-demo` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/sablier_metamask_demo/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns sablier-metamask-demo.example
  (:require [sablier-metamask-demo.config :as config])
